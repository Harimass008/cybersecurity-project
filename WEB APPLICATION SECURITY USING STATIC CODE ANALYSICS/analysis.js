...
const API_KEY = "AIzaSyCq6NCb8HPbaA_d9LSB1nr3UaDY1VmYu4c";
...
const loadDataFromLocalstorage = () => {
    ...
    // Load the code from localStorage
...
const generateAPIResponse = async (incomingMessageDiv) => {
  ...
  try {
      ...
      if (apiResponse.includes("```")) {
          const suggestedCode = apiResponse.match(/```[\s\S]*?```/)[0].replace(/```/g, '').trim();
...
const applyCodeSuggestion = (suggestedCode) => {
  ...
  localStorage.setItem('fileContent', updatedCode);
}
...
// Function to analyze code and suggest improvements
const analyzeCode = async () => {
  const code = codeBlock.textContent;
  userMessage = `Kindly give the vulnerability rectified code only:\n\n${code}`;
  handleOutgoingChat();
}
